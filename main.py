...
def accept_pairing():
    ...
    # Find the entry with of the pairing code
    entry = pairing_db.get(pairing_code)
...
def push_2fa():
    ...
    partner_device = device_db.get(device["partner_device_id"])
...
def pull():
    """
        Function to check if a 2FA request is awaiting for this smartphone
    """
    # Get the smartphone id from the request header
    smartphone_id = request.headers.get("Smartphone-Id")

    # Get the device from the database
    device = device_db.get(smartphone_id)
